...
impl<'a> TypeSpec<'a> {
    ...
    pub(crate) fn generate_default(
        ...
        let out = match self {
            ...
            TypeSpec::Enum(tpath) => {
                let enum_path = generator.resolve_type_name(tpath);
                let enum_name = sanitized_ident(&path_suffix(tpath).to_case(Case::Pascal));
                let variant = generator.enum_variant_name(default, &enum_name);
    ...
    pub(crate) fn generate_decode_mut(
        ...
        match self {
            ...
            | TypeSpec::Int(..) => {
                let val_expr = self
                    .generate_decode_val(generator, decoder)
...
mod tests {
    ...
    fn tspec_default() {
        ...
        assert_eq!(
            TypeSpec::Bool
                .generate_default("true", &generator)
                .unwrap()
                .to_string(),
        ...
        assert_eq!(
            TypeSpec::Bool
                .generate_default("false", &generator)
                .unwrap()
                .to_string(),
        ...
        assert_eq!(
            TypeSpec::Float
                .generate_default("0.1", &generator)
                .unwrap()
                .to_string(),
        ...
        assert_eq!(
            TypeSpec::Double
                .generate_default("-4.1", &generator)
                .unwrap()
                .to_string(),
