...
fn type_check() {
    ...
    let _: heapless::String<4> = data.s;
    let _: heapless::Vec<u8, 6> = data.b;

    let list = proto::List::default();
    let _: heapless::Vec<_, 4> = list.list;

    let strlist = proto::StrList::default();
    let _: heapless::Vec<heapless::String<4>, 2> = strlist.list;

    let fixedlist = proto::FixedList::default();
    let _: heapless::Vec<_, 4> = fixedlist.list;

    let numlist = proto::NumList::default();
    let _: heapless::Vec<_, 8> = numlist.list;

    let enumlist = proto::EnumList::default();
    let _: heapless::Vec<_, 2> = enumlist.list;

    let map = proto::Map::default();
    let _: std::collections::BTreeMap<heapless::String<4>, heapless::Vec<u8, 3>> = map.mapping;
